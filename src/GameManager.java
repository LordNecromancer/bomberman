...
//
        ...
        addToGameTable(createBoard, statement, connection, name);
        addToGameSpecs(createBoard, connection);
        addToGameComponentsArray(createBoard, connection);
    }

    private static void addToGameComponentsArray(CreateBoard createBoard, Connection connection) {
        Statement statement;
        try {
            statement = connection.createStatement();
            ...
            insertIntogameComponentTable(createBoard, connection, str2);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

    private static void insertIntogameComponentTable(CreateBoard createBoard, Connection connection, String str2) throws SQLException {
        ...
        for (int i = 0; i < createBoard.width + 2; i++) {
            for (int j = 0; j < createBoard.height + 2; j++) {
                ...
                preparedStatement.setString(2, createBoard.gameComponents[i][j].getClass().getName());
...
//        }
    ...
    private static void addToGameSpecs(CreateBoard createBoard, Connection connection) {
        Statement statement;
        try {
            statement = connection.createStatement();
            ...
            insertIntoGameSpecsTable(createBoard, connection);
    ...
    private static void insertIntoGameSpecsTable(CreateBoard createBoard, Connection connection) throws SQLException {
        String str2 = "insert into bomberManSpecs (gameId,bombSet,bombControl,playerSpeed,isAlive,bombNum,bombCount,gameTime,score) " + "values (?,?,?,?,?,?,?,?,?)";

        PreparedStatement preparedStatement = connection.prepareStatement(str2);
        preparedStatement.setInt(1, gameId);
        preparedStatement.setBoolean(2, createBoard.player.bombSet);
        preparedStatement.setBoolean(3, createBoard.player.bombControl);
        preparedStatement.setInt(4, (int) createBoard.player.playerSpeed);
        preparedStatement.setBoolean(5, createBoard.player.isAlive);
        preparedStatement.setInt(6, createBoard.bombNum);
        preparedStatement.setInt(7, createBoard.bombCount);
        preparedStatement.setInt(8, (int) createBoard.gameTime.getTime());
        preparedStatement.setInt(9, createBoard.points);


        preparedStatement.execute();
    }

    private static void addToGameTable(CreateBoard createBoard, Statement statement, Connection connection, String name) {
        try {
            ...
            PreparedStatement preparedStatement = insertIntoGameTable(createBoard, connection, name);
    ...
    private static PreparedStatement insertIntoGameTable(CreateBoard createBoard, Connection connection, String name) throws SQLException {
        ...
        preparedStatement.setInt(2, createBoard.width);
        preparedStatement.setInt(3, createBoard.height);
...

    {
        ...
        try {
            ...
            while (result.next()) {
                String type = result.getString("objectName");
            ...
            gameManager.createBoard.bombCount = bombCount;
            gameManager.createBoard.bombNum = bombNum;
            gameManager.createBoard.gameTime = new Time(gameTime);
            gameManager.createBoard.points = score;
            gameManager.init();
